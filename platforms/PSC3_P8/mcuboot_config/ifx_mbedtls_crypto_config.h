 ...
 *
 * Note: The bootloader uses Cryptolite hardware for ECDSA verification
 *       (ecdsa_cryptolite.c), not this mbedTLS module. Only enable when
 *       USE_CRYPTOLITE_ECDSA is defined to avoid pulling in ECC prerequisites
 *       for non-ECDSA configurations (LMS, XMSS, or all features disabled).
 *
 * MBEDTLS_ECP_C is required by MBEDTLS_ECDSA_C. 
 * Without encryption (no PSA_WANT_ALG_ECDH), the PSA
 * config only auto-derives MBEDTLS_ECP_LIGHT (from ECC_PUBLIC_KEY), not
 * the full MBEDTLS_ECP_C. We must define it explicitly here.
 */
...
/* Disable precomputed comb tables for fixed-point ECP multiplication.
 * Default is 1, which includes secp256r1_T[16]. Since P-256 ECDH uses 
 * Cryptolite hardware, these software ECP tables are redundant.  
 * Setting to 0 trades for the size saving — acceptable because Cryptolite
 * handles the actual ECDH computation. */
 ...
 * 32 keys.
 *
 * Bootloader needs at most ~5 concurrent PSA keys (ECDH, AES-CTR, HMAC,
 * CMAC, ECDSA). Reducing from 32 to 8 saves ~960 bytes of BSS.
 */
